...
@section('title', 'Cập Nhật Chi Nhánh - ClinicHub')

@section('content')
    <div class="container-fluid py-3">
        <div class="d-flex justify-content-between align-items-center mb-4 px-4">
            <div>
                <h5 class="mb-1 fw-bold text-dark">{{ __('Edit Branch') }}</h5>
                ...
                <form action="{{ route('branch.update', $branch->id) }}" method="POST">
                    ...
                    <div class="row g-3">
                        <div class="col-12 mb-2">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $branch->name) }}" placeholder="Ví dụ: Phòng khám ClinicHub Cơ sở 1..." required>
                            @error('name')
                        ...
                        <div class="col-md-6 mb-2">
                            ...
                            <input type="text" class="form-control @error('code') is-invalid @enderror" id="code" name="code" value="{{ old('code', $branch->code) }}" placeholder="Ví dụ: CN01, CN-HN..." style="text-transform: uppercase;" required>
                            @error('code')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6 mb-2">
                            ...
                            <input type="text" class="form-control @error('phone') is-invalid @enderror" id="phone" name="phone" value="{{ old('phone', $branch->phone) }}" placeholder="Nhập số điện thoại chi nhánh">
                            @error('phone')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6 mb-2">
                            ...
                            <select name="province_id" id="province_select" class="form-select @error('province_id') is-invalid @enderror">
                                ...
                                @foreach($provinces as $province)
                                    <option value="{{ $province['idProvince'] }}" {{ old('province_id', $branch->province_id) == $province['idProvince'] ? 'selected' : '' }}>
                                        {{ $province['name'] }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-12 mb-2">
                            ...
                            <input type="text" class="form-control @error('address_detail') is-invalid @enderror" id="address_detail" name="address_detail" value="{{ old('address_detail', $branch->address_detail) }}" placeholder="Số nhà, tên đường, tòa nhà...">
                        ...
                        <div class="col-md-6 mb-2">
                            ...
                            <select name="status" id="status" class="form-select">
                                <option value="1" {{ old('status', $branch->status) == '1' ? 'selected' : '' }}>Hoạt động</option>
                                <option value="0" {{ old('status', $branch->status) == '0' ? 'selected' : '' }}>Ngừng hoạt động (Khóa)</option>
                            </select>
                        </div>
                    </div>
...
@section('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const currentCommuneId = "{{ old('commune_id', $branch->commune_id) }}";
            ...
            if (provinceSelect.value) {
                loadCommunes(provinceSelect.value, currentCommuneId);
            }
        });
    </script>
@endsection
